...
#
# Two commands run in this container: `srcds_linux`, the game, and
# `srcds_run`, the script that restarts it. Both start with `srcds`, so a
# check for that substring is satisfied by either.
#
...
EXACT='grep -qsx srcds_linux /proc/[0-9]*/comm'
LOOSE='grep -qs srcds /proc/[0-9]*/comm'
...
docker exec "$RUN" sh -c 'for p in /proc/[0-9]*; do
  [ "$(tr "\0" "\n" < "$p/cmdline" 2>/dev/null | head -1)" = "/tmp/srcds_linux" ] && kill -9 "${p##*/}" 2>/dev/null
...
for _ in $(seq 1 20); do
  n="$(docker exec "$RUN" sh -c 'c=0; for p in /proc/[0-9]*; do
        [ "$(tr "\0" "\n" < "$p/cmdline" 2>/dev/null | head -1)" = "/tmp/srcds_linux" ] && c=$((c+1)); done; echo "$c"' 2>/dev/null)"
...
if docker exec "$RUN" sh -c 'for p in /proc/[0-9]*; do [ "$(tr "\0" "\n" < "$p/cmdline" 2>/dev/null | head -1)" = "/tmp/srcds_run" ] && exit 0; done; exit 1' 2>/dev/null; then
